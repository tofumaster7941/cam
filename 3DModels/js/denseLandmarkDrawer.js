...
export class DenseLandmarkDrawer {
     ...
     */
    drawBodyConnections(ctx, bodyPoints, w, h) {
        if (!bodyPoints) return;

        // Create lookup by original index
        const lookup = {};
        ...
        for (const [a, b] of connections) {
            ...
            gradient.addColorStop(0, this.getColor(pa));
            gradient.addColorStop(1, this.getColor(pb));
     ...
     */
    drawHandConnections(ctx, handPoints, w, h) {
        if (!handPoints) return;

        // Group points by hand
        const hands = {};
        handPoints.forEach(p => {
            const handKey = p.handIndex || 0;
            if (!hands[handKey]) hands[handKey] = {};
            if (p.originalIndex !== undefined) {
                hands[handKey][p.originalIndex] = p;
            }
        });
        ...
        for (const [handIdx, lookup] of Object.entries(hands)) {
            const color = Object.values(lookup)[0]?.region?.includes('Left')
                ? this.colors.hand_Left
                : this.colors.hand_Right;
    ...
    drawInfoPanel(ctx, data) {
        ...
        const stats = [
            ...
            `├ Body: ${data.body?.count || 0}`,
            `└ Hands: ${data.hands?.count || 0}`,
    ...
    draw(processedData) {
        ...
        if (this.showConnections) {
            ...
            if (processedData.body?.points) {
                this.drawBodyConnections(ctx, processedData.body.points, w, h);
            }
            if (processedData.hands?.points) {
                this.drawHandConnections(ctx, processedData.hands.points, w, h);
            }
        }
        ...
        this.drawRegionPoints(ctx, processedData.body?.points, w, h, showInterpolated);
        this.drawRegionPoints(ctx, processedData.hands?.points, w, h, showInterpolated);
